...
2. 같은 실행에서 생성된 ID가 모두 다른지 출력으로 확인하세요.
3. `spawn_empty()` 호출을 함수로 분리해 보세요.

## 심화 과제

`app.world_mut().despawn(entity)`를 이용하는 별도 실험을 작성해 Entity를 제거하세요. 제거한 ID에 대해 `world.get_entity(entity)`의 결과가 어떻게 달라지는지 확인하세요.
