...
회전 장애물은 world Transform을 반영해 베이크합니다. 이동 장애물은 매 프레임 전체 재베이크보다 지역 회피, 타일 재베이크, 링크 on/off 중 요구사항에 맞는 방식을 고릅니다.

문은 링크 또는 Island 연결을 상태에 따라 비활성화하고 경로를 다시 요청합니다. 수행 예제는 문이 열렸을 때 직선 경로, 닫혔을 때 복도 우회 경로를 선택하는지 검사합니다.
